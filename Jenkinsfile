...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="jaminism/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/jaminism/guestbook.git'
            }
        }
        stage('Build') {
            steps {
            ...
            steps{
                ...
                withSonarQubeEnv('SonarQube-Server'){
                    sh '''
                        ...
                        -Dsonar.login=def702688be49cec97a67e5a43f8f9396b5c4d1d
                    '''
                }
                */
            }
    ...
    post { 
        always { 
            emailext (attachLog: true, body: '본문', compressLog: true
                    , recipientProviders: [buildUser()], subject: '제목', to: 'jaminism.j@gmail.com')
